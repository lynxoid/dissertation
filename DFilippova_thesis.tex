...
In this dissertation, we explore solutions to the data storage problem and suggest approaches to sequence and sequence alignment compression. Specifically, we consider a problem of identifying information shared across multiple related datasets and its applications to sequence compression. We formulate the problem of encoding the data given such shared information and provide an optimal algorithm for doing so. Further, we develop a functional compression scheme for sequence alignments that allows for fast computations on compressed sequence and modular data downloads. 
...
Additionally, we investigate the problem of visualizing large multi-dimensional datasets where we are interested in evaluating the robustness of its suggested structure. We develop novel visualizations and processes to evaluate robustness of such annotations and study the example of protein function imputation for a PPI network of an \textit{A. thaliana} plant.
We also investigate challenges of visualizing such complex data when data is changing over time.
...
The approaches we developed are scalable and can be applied more broadly to other subject areas. %(XXX horribly general conclusion)
...
Visualization is an important first step when working with a new set of data as it provides the means to assess the overall shape of the data, its distribution, and to detect outliers. Anscombe's quartet~\cite{anscombe} succinctly demonstrates the importance of viewing the data before analyzing it: four datasets have the same $x$ and $y$ mean and variance, Pearson's correlation and fit the same regression line, yet are strikingly different (Figure~\ref{fig:intro:anscombe}). We see visualization as the means to generate new hypotheses that are then tested and validated using complex algorithmic and statistical models.
...
\begin{itemize}
  ...
  We develop new and adapt existing visualizations useful for evaluation and comparison of collections of annotations derived on large noisy data through various classification strategies. We integrate these visualizations into a single application that allows to perform a variety of analyses on these classification results and automatically determines annotations that are most consistent across all sets of assigned labels. We use the tool to perform a detailed analysis of clusters of protein-protein interactions in \textit{A. thaliana} plant and identify robust cluster cores that are significantly enriched for Gene Ontology~\cite{GeneOntrology} terms.
  ...
  We extend the algorithm for identifying robust subsets of annotations from our earlier work to the problem of finding densely packed areas of chromatin (\textit{topological domains}). Our algorithm is faster, requires less parameters, and generates domains that are significantly different from domains identified by existing methods, however, are more enriched for the chromatin markers associated with gene regulation. We obtain domains at different scales that allows us to evaluate, for the first time, the hierarchical structure of the chromatin.

  \item \textbf{Compression methods for sequencing and alignment data}

  % Huffmer, Referee XXX
  We propose novel ways to compress sequencing data using information shared across several related datasets and develop new algorithms for rapid compression of sequence alignments. We develop an infrastructure where related alignment data are separated into disjoint streams allowing for better compression within the stream. Additionally, separable streams  allow us to perform computations on parts of the data without fully reconstructing the original alignments speeding up certain analyses by orders of magnitude.
...
% The amount of sequencing data stored in the Short Read Archive (SRA) is 775337Gb and growing~\cite{SRA}. 
...
\section{Compressing sequence with standard command-line tools}
  ...
  \begin{table}[ht]
    ...
    \caption{\textbf{Performance of command-line compression tools on sequencing reads.} Reordering the reads improves reads compression by up to 6 times. Sizes shown are in megabytes.}
...
This approach was able to reduce the file size to a factor of 1/7th of the original size and, in certain instances, even outperformed state of the art tools (see Figure~\ref{fig:denovocompr:varyK}).
  ...
  % XXX find dictionary sizes.

  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  \subsection{Building dictionaries to maximize entropy}
  % \subsection{Selecting kmers based on their frequencies in the de Bruijn graph}

  XXX discuss entropy measures
  ...
  % XXX: plot of non-overlapping vs overlapping kmer frequency distributions
  ...
  % We will implement and investigate the performance of this algorithm and establish its relationship to vertex and set cover problems. 

  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  % \subsection{Building a dictionary with kmers of mixed lengths}

  % To ensure that mixed-length kmers are selected into $D$, we can maintain several graphs $G_{k-1}(\seq), G_{k-2}(\seq), \ldots$ and prune them in a manner consistent with pruning in $G_{k}(\seq)$.
...
\section{Results}
  ...
  % performance on some self-similar data:
  ...
  \subsection{Performance when building the dictionary and compressing across related datasets}
    ...
    %   -- but the reads do align well to the genome
    ...
    \begin{table}
      ...
      \begin{tabular}{l c r r r r r}
          ...
          *  & Cell type & Read count & $|r|$ & $D \cap T$ & $D \backslash T$ & $T \backslash D$ \\
          ...
          \bottomrule
      \end{tabular}
      \caption{\textbf{Number of kmers shared between human RNA-seq experiments and annotated human transcriptome.} Here, $D$ is the set of kmers counted on the corresponding dataset, $T$ -- kmers from the transcriptome. The transcriptome had a total of 115mln kmers.}
...
Our main contribution is the notion of separable streams of data. \refer groups sequence and alignment data into separate streams that allows for more efficient compression due to increased data homogeneity within the stream. The streams of different kinds of data are independent and can be downloaded and reconstructed independently. Most importantly, due to the stream's smaller size and simple structure, \refer allows for operations on the sequence data stream that are orders of magnitude faster than equivalent commands in the commnly used package \texttt{samtools} 
...
\section{Quality values compression}
  ...
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  \subsection{Indexing for random access and partial decompression}
...
ooutline some future developments for each contribution?
