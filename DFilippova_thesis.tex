...
Commonplace compression tools like gzip and bzip based on Lempel-Ziv original paper~\cite{LempelZiv} are used often to compress large read datasets, however, they do not exploit the specific nature of sequencing reads: its small alphabet, highly repetitive subsequences, and the fact that sequence may originate from a well-annotated known genome. SRA data format is commonly used to store FASTQ files which include the reads themselves and additional base quality information, however, even the most naive compression methods easily outperform this compression technique~\cite{CompressionSqueezeCompetition}.
...
\section{Background}
  ...
  The amount of sequencing data stored in the Sequence Read Archive (SRA) is 775337Gb and growing~\cite{SRA} --- this growth of generated data demands novel scalable approaches to its storage and transmission. It is also crucial compression approaches are \textit{functional}, i.e.\@that analyses could be run on the dataset in its compressed format without having to fully reconstruct it. 
...
\section{De novo sequence compression}
  ...
  The repetitive nature of sequencing data suggests that dictionary-based coders, like the commonplace command-line tools \texttt{gzip}, \texttt{bzip}, and \texttt{plzip}, should be able compress collections of reads reasonably well.
  ...
  \subsection{Builiding efficient dictionaries}
